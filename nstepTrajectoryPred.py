...
def split_seq(seq,window,horizon):
  ...
  scaled_data = seq[['x','y']].values
  X = []; Y = []
  for carIdx in range(0, len(scaled_data), 50) : 
...
def train(trainX, trainY, model, num_epochs) : 
  ...
  for t in range(num_epochs):
    ...
    if t % 10 == 0 and t != 0:
      print("Epoch ", t, "RMSE: ", torch.sqrt(loss).item())
      print("x_loss : ", torch.sqrt(x_loss).item())
      print("y_loss : ", torch.sqrt(y_loss).item())
...
if __name__ == '__main__':
  ...
  horizon = 5 #얼마나 먼 미래? #마지막 프레임의 위치 예측
  ...
  # # todo 모델 저장
  torch.save(model, './camera1/model/dt1215_win_25_step_17_model_e200_d1800.pt')
  # model = torch.load('./camera1/model/dt1215_win_25_step_17_model_e200_d1800.pt').to(device)
  # model = load_state_dict(torch.load('./camera1/model/win_25_step_17_model_e2000_d1800.pt', map_location='cuda:0'))
